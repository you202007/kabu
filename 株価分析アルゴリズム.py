...
class OmnisEngineV5_2:
    ...
    def breakout_volume_features(self, df, lookback=20):
        ...
        prev_high = np.nanmax(close[-(lookback + 1):-1])
    ...
    def analyze_strategy_1(self, df):
        ...
        vol_boost = 0.0 if np.isnan(vol_ratio) else np.tanh((vol_ratio - 1.0) * 0.8)
        s_break = breakout
...
def run_simulation():
    ...
    if not df_s1.empty:
        ...
        df_s1.to_csv(OUTPUT_S1, index=False, encoding='utf-8-sig')                          
        df_s1.to_csv(f"{HISTORY_DIR}s1_{today_str}.csv", index=False, encoding='utf-8-sig') 
    ...
    if not df_s2.empty:
        ...
        df_s2.to_csv(OUTPUT_S2, index=False, encoding='utf-8-sig')                          
        df_s2.to_csv(f"{HISTORY_DIR}s2_{today_str}.csv", index=False, encoding='utf-8-sig') 
